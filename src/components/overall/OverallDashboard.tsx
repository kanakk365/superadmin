            ...
            <div className="flex flex-col gap-6 lg:gap-8 px-4 sm:px-8 py-8 max-w-[1600px] mx-auto">
                ...
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-muted-foreground">
                        Global insights and performance metrics across all connected platforms.
                    </p>
                </div>
                    ...
                    {overallStats.map((stat, index) => (
                        <div
                            ...
                            className={cn(
                                ...
                                stat.color === "text-emerald-500" && "hover:shadow-emerald-500/10",
                                stat.color === "text-blue-500" && "hover:shadow-blue-500/10",
                                stat.color === "text-purple-500" && "hover:shadow-purple-500/10",
                                stat.color === "text-amber-500" && "hover:shadow-amber-500/10"
                            )}
                        >
                            <div className="flex items-start justify-between">
                                <div className={cn("rounded-2xl p-3 transition-colors duration-300 group-hover:scale-110", stat.bg)}>
                                ...
                                {stat.trend !== "neutral" && (
                                    <div
                                        className={cn(
                                            ...
                                            stat.trend === "up" || (stat.title.includes("Alerts") && stat.trend === "down")
                ...
                <div className="flex flex-col gap-4">
                    <div className="flex items-center justify-between">
                        <h2 className="text-xl font-semibold tracking-tight">Project Summary</h2>
                    </div>
                ...
                <div className="grid gap-6 lg:grid-cols-3">
                    ...
                    <Card className="lg:col-span-2 rounded-[32px] border-border/40 shadow-sm bg-card/50 backdrop-blur-sm">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold tracking-tight text-foreground">Global Traffic & User Activity</CardTitle>
                    ...
                    <Card className="rounded-[32px] border-border/40 shadow-sm flex flex-col bg-card/50 backdrop-blur-sm">
                        <CardHeader>
                            <CardTitle className="text-xl font-bold tracking-tight text-foreground">Revenue Growth</CardTitle>
                            <CardDescription className="text-sm text-muted-foreground">6-Month financial performance.</CardDescription>
                        </CardHeader>
                            ...
                            <div className="h-[250px] w-full">
                                <ResponsiveContainer width="100%" height="100%">
                                    <AreaChart data={revenueTrendData}>
                                        ...
                                        />
                                        <Area type="monotone" dataKey="value" stroke="#8b5cf6" strokeWidth={3} fill="url(#revenueGradient)" />
                                    </AreaChart>
                                </ResponsiveContainer>
                            </div>
                        </CardContent>
...
// Replaced the small 'ProjectCard' with this larger, premium card design
// matching the 'Revenue Card' from BattleLounge/DestinationKP/YSN stats.
const LargeProjectCard = ({ project }: { project: any }) => {
    // Generate simple chart data based on the project data
    const chartData = project.data.map((val: number, i: number) => ({ name: i, value: val }));
            ...
            <div className="flex flex-col h-full justify-between relative z-10">
                ...
                <div className="grid grid-cols-3 gap-4 pt-4 border-t border-white/20">
                    <div className="flex flex-col gap-1">
                        ...
                        <span className="text-xl font-bold text-white">{project.revenue}</span>
                    </div>
                    <div className="flex flex-col gap-1 border-l border-white/20 pl-6">
                        <span className="text-xs font-semibold text-white/60 uppercase tracking-wider">
                            Users
                        </span>
                        <span className="text-xl font-bold text-white">{project.users}</span>
                    </div>
                    <div className="flex flex-col gap-1 border-l border-white/20 pl-6">
                        <span className="text-xs font-semibold text-white/60 uppercase tracking-wider">
                            Active
                        </span>
                        <span className="text-sm font-bold text-white mt-1 line-clamp-1">{project.activeItem}</span>
                    </div>
                </div>
            </div>
